...
7. **GUI 登录/拉起链路改动必须两态真机验证**：全 mock 单测覆盖不了首次运行路径（新机器、无登录态、杀软首扫、扫码等待）。改 login/setup/接管/Job 链路后，发布前必须：a) 把 `~/.boss-zhipin-scraper` 挪走模拟新电脑，跑「开始采集」全链路（向导弹出、登录页保持、日志滚动、停止有效），验完恢复；b) 已登录态直接跑（自动续跑、数据落盘）。
