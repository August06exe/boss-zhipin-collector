...
7. **GUI 登录/拉起链路改动必须两态真机验证**：单测全 mock 覆盖不了首次运行路径（新机器、无登录态、杀软首扫、扫码等待）。改 login/setup/接管/Job 链路后，发布前必须做：a) 把 `~/.boss-zhipin-scraper` 挪走模拟新电脑，跑「开始采集」全链路（向导弹出、登录页保持、日志滚动、停止有效），验完恢复档案；b) 已登录态直接跑（自动续跑、数据落盘）。2026-09-24 教训：一天连发 6 版单测全绿，真机连续暴雷 5 次（浏览器被 Job 陪葬、向导因措辞漏判不弹、等待期无界面）。
8. **合规红线**：只做被动捕获（Network 域旁听页面自身请求）和请求节奏控制。**禁止**主动伪装类手段：指纹伪造、验证码绕过、代理池轮换。遇到 `_security_check` 验证页时提示用户人工处理，不自动绕过。
